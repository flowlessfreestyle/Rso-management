...
export default function EventsClient({ events, userId }: { events: Event[], userId: string }) {
  ...
  const renderEventCard = (event: Event, isPast: boolean = false) => {
    ...
    return (
      ...
      >
        <div className="p-6">
          <h3 className={`text-xl font-bold mb-2 ${isPast ? 'text-gray-500' : 'text-gray-900'}`}>
            {event.title}
          </h3>
          
          <div className="space-y-2 mb-4">
            <div className={`flex items-center text-sm ${isPast ? 'text-gray-400' : 'text-gray-600'}`}>
          ...
          {event.description && (
            <p className={`text-sm mb-4 line-clamp-2 ${isPast ? 'text-gray-400' : 'text-gray-600'}`}>
              {event.description}
            </p>
          )}
